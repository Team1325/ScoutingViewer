...
.controller('StatisticsCtrl', ['$scope', '$state', '$http', '$rootScope', function($scope, $state, $http, $rootScope) {
  ...
  $scope.newTeam = function(filterTeam) {
    ...
    $scope.teams =    {team: 0,
        ...
        $scope.teams.teleopCycleTimeAverage = $scope.teams.teleopCycleTimeTotal/theUltimateCycleCount;
...
.controller('RankingsCtrl', ['$scope', '$state', '$http', '$rootScope', '$filter', function($scope, $state, $http, $rootScope, $filter) {
  ...
  $scope.rankList = function(filterStat, reverseOrder) {
        ...
        $scope.teams.teleopTotalDamageAverage = $scope.teams.teleopTotalDamageTotal/$scope.teams.totalGameCount;
        $scope.teams.teleopCycleTimeAverage = $scope.teams.teleopCycleTimeTotal/theCycleCount;
        ...
        else if (filterStat == "Teleop Avg High Goal")
        {
          $scope.teams.filterVarStat = $scope.teams.teleopHighBallAverage;
        }
        else if (filterStat == "Teleop Avg Low Goal")
        {
          $scope.teams.filterVarStat = $scope.teams.teleopLowBallAverage;
        }
        ...
        {
          $scope.allStats = $filter('orderBy')($scope.allStats, "teleopHighBallAverage", (!reverseOrder));
          $scope.filterDisplayStat = "Avg High Goal: ";
          $scope.filterDisplayUnit = "";
          console.log("Ranking...  Reversed: " + (reverseOrder));
          console.log("Ordered Teams:")
          for (dsa = 0; dsa<$scope.allStats.length; dsa++)
          {
              console.log(JSON.stringify($scope.allStats[dsa].teleopHighBallAverage));
          }
        }
        else if (filterStat == "Teleop Avg Low Goal")
        {
          $scope.allStats = $filter('orderBy')($scope.allStats, "teleopLowBallAverage", (!reverseOrder));
          $scope.filterDisplayStat = "Avg Low Goal: ";
          $scope.filterDisplayUnit = "";
          console.log("Ranking...  Reversed: " + (reverseOrder));
          console.log("Ordered Teams:")
          for (dsa = 0; dsa<$scope.allStats.length; dsa++)
          {
              console.log(JSON.stringify($scope.allStats[dsa].teleopLowBallAverage));
          }
        }
...
.controller('AllianceCtrl', ['$scope', '$state', '$http', '$rootScope', function($scope, $state, $http, $rootScope) {
  ...
  $scope.newTeam = function(filterTeam, teamPosition) {
    ...
    $scope.teams =    {team: 0,
        ...
        if (teamPosition==1)
        {
          $scope.teamsOne = $scope.teams;
        }
        else if (teamPosition==2)
        {
          $scope.teamsTwo = $scope.teams;
        }
        else if (teamPosition==3)
        {
          $scope.teamsThree = $scope.teams;
        }
